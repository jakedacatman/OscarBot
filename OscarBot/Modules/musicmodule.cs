...
namespace OscarBot.Modules
{
    ...
    public class MusicModule : InteractiveBase<ShardedCommandContext>
    {
        ...
        public MusicModule(LavalinkManager manager, DiscordShardedClient client, MiscService misc, MusicService ms, DbService db)
        {
            _manager = manager;
            _client = client;
            _misc = misc;
        ...
        public async Task AddCmd([Summary("The URL to add from YouTube, or search term."), Remainder]string urlOrTerm)
        {
            ...
            try
            {
                ...
                if (Uri.TryCreate(urlOrTerm.Trim('<', '>'), UriKind.Absolute, out Uri uriResult) && (uriResult.Scheme == Uri.UriSchemeHttp || uriResult.Scheme == Uri.UriSchemeHttps))
                {
                    ...
                    using (WebClient client = new WebClient())
                    {
                        ...
                        await (await _ms.GetGuildQueue(Context.Guild.Id)).QueueUp(Context.Guild.Id, s);
        ...
        public async Task SkipCmd()
        {
            try
            {
                ...
                await users.AddUser(new Skip { SongUrl = song.URL, UserId = currUser.Id }, Context.Guild.Id);

                if (currUser.Id == song.QueuerId)
                {
                    await users.RemoveUsers(Context.Guild.Id);
        ...
        private async Task TrackEnd(LavalinkPlayer player, LavalinkTrack oldTrack, string reason)
        {
            ...
            var song = await queue.Pop(Context.Guild.Id);
